...
if names_list:  # Check if names_list is not empty
    ...
    if any(name for name in names_list):
        ...
        names_md = ', '.join([f'<span style="color:{color};"><strong><u>{name}</u></strong></span>' if len(name.split()) > 4 or len(name.split()) < 2 or any(len(word) < 3 for word in name.split()) or (re.search(r'[^a-zA-Z\s]', name) and '(Marked As Not Present)' not in name) else f'<span style="color:{color};">{name}</span>' for name, color in formatted_names])
